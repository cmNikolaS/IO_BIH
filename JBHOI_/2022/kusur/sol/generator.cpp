...
void stopMeasuringTime() {
    ...
    timeMeasurementMS = std::chrono::duration_cast<std::chrono::milliseconds>(end - timeMeasurementStart).count();
}
...
std::function<bool(std::ifstream&, int, int)> outputCallback = nullptr;
...
void generateSubtask(int subtask, int testcase, float testPercent) {
    ...
    do {
        ...
        if (outputCallback != nullptr) {
            std::ifstream outputFile(outputFileName.str());
            finish = outputCallback(outputFile, subtask, testcase);
        }
    } while(!finish);
    globalCount++;
    std::cout << timeMeasurementMS << "ms\n";
}
